...
async def start(_, message):
    ...
    if await CustomFilters.authorized(_, message):
        ...
        await send_message(
            message,
            "<i>Welcome to Mega Renamer Bot!....</i>",
            reply_markup,
        )
    else:
        await send_message(
            message,
            "<i>Bot can rename File(s), Folder(s) in MEGA Account within Seconds.\n\n⚠️ You Are not authorized user!</i>",
            reply_markup,
        )
...
async def log_cb(_, query):
    ...
    if user_id != int(data[1]):
        ...
        try:
            ...
            log_content = "\n".join(reversed(res))
            escaped_log = escape(log_content)

            text = (
                f"<b>Showing Last {len(res)} Lines from log.txt:</b>\n\n"
                f"----------<b>START LOG</b>----------\n\n"
                f"<blockquote expandable>{escaped_log}</blockquote>\n"
                f"----------<b>END LOG</b>----------"
            )
